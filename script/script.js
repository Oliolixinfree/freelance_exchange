document.addEventListener('DOMContentLoaded', () => {
    ...
    const modarOrderActive = document.getElementById('order_active');
    ...
    const decOfNum = (number, titles) => number + ' ' + titles[(number % 100 > 4 && number % 100 < 20) ? 2 : [2, 0, 1, 1, 1, 2][(number % 10 < 5) ? number % 10 : 5]];
    ...
    // функция, которая берет заказы из массива orders и составляет таблицу во вкладке фрилансер
    const renderOrders = () => {
    ...
    // функция, которая открывает модальное окно с заказом
    const openModal = (numberOrder) => {
        const order = orders[numberOrder];

        // деструктуризация modal
        const {
            title,
            firstName,
            email,
        ...
        const modal = active ? modarOrderActive : modalOrder;
    ...
    formCustomer.addEventListener('submit', (event) => {
        ...
        // собираются все данные из формы, перебираются, то, что не нужно отсеивается и добавляется в объект
